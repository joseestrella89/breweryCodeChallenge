...
import IBreweryRepository, {
    BreweryListResponse,
    SearchByParams,
    FilterByParams,
} from '@Services/repositories/IBreweryRepository';
...
export default class BreweryRepositoryNetwork implements IBreweryRepository {
    ...
    async filterBy(params: FilterByParams): Promise<BreweryListResponse> {
        ...
        const breweries = breweriesDTO.map((breweryDTO: BreweryDTO) =>
            this.breweryMapper.mapBrewery(breweryDTO),
        );
        return { breweries };
    }

    async searchByKeyword(params: SearchByParams): Promise<BreweryListResponse> {
        const breweriesDTO = await this.networkService.get<BreweryDTO[]>(
            `${Envs.API_URL}/breweries/autocomplete?query=${params.query}&per_page=${params.perPage}`,
        );
        const breweries = breweriesDTO.map((breweryDTO: BreweryDTO) =>
            this.breweryMapper.mapBrewery(breweryDTO),
        );
